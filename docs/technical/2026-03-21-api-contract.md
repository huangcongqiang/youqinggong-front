...
- 当前至少以下 `spring-app` 路径已经按 token 解 actor，而不是信任 body/query 里的当前操作者身份：
  ...
  - `POST /tasks/{taskId}/negotiations/confirm`（兼容接口，非前台主链）
...
- 如果任务已经进入任务确认单流程（存在 `taskConfirmationJson`），该接口会显式返回 `actionBlocked=true`，不再推进主链
...
| `downloadUrl` | `string` | 否 | 下载地址，`spring-app` 当前可从上传会话补齐 |
| `size` | `number` | 否 | 文件大小 |
...
```json
{
  ...
  "fileType": "video",
  "objectKey": "task-001/2026/03/demo.mp4",
  "downloadUrl": "https://cdn.example.com/task-001/2026/03/demo.mp4",
  "size": 24800000,
  "mimeType": "video/mp4",
