...
assertEqual(
  resolveApiBase({}, createRuntime({ search: '?apiTarget=mock', hostname: 'm.site.local' })),
  'http://m.site.local:8080/api',
  'resolveApiBase should use the root mock port when apiTarget=mock'
);

assertEqual(
  resolveApiBase({ VITE_API_TARGET: 'spring' }, createRuntime({ hostname: 'm.site.local' })),
  'http://m.site.local:8081/api',
  'resolveApiBase should use the spring-app port when the runtime target is spring'
);

assertEqual(
  resolveApiBase({}, createRuntime({ hostname: 'm.site.local' })),
  'http://m.site.local:8081/api',
