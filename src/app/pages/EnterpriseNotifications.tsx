...
import { Bell, CheckCircle, Clock, FileText, AlertCircle, ArrowRight, MessageSquare } from 'lucide-react';
import { Link } from 'react-router';
import { getBusinessData } from '../services/api';
import { asArray, stringOf } from '../services/workflowFormatters';
import { EmptyState, ErrorState, LoadingState } from '../components/AsyncState';

function buildEnterpriseNotificationLink(item: any) {
  const taskId = stringOf(item?.taskId, item?.recordId);
  const roomKey = stringOf(item?.roomKey, item?.room);
  const source = stringOf(item?.source, item?.groupKey, 'notifications').toLowerCase();
  ...
  if (source === 'records') {
    return `/enterprise/records/${encodeURIComponent(taskId)}${suffix}`;
  }
...
function normalizeNote(item: any, index = 0) {
  const source = stringOf(item?.source, item?.groupKey, 'notifications');
  return {
    ...
    link: buildEnterpriseNotificationLink(item)
  };
}
...
export function EnterpriseNotifications() {
  ...
  useEffect(() => {
    ...
    getBusinessData().then((data: any) => {
      if (!alive) return;
      setNotifications(asArray<any>(data?.notificationItems || data?.attentionItems).map(normalizeNote));
