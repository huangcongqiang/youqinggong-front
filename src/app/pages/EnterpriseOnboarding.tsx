    ...
    <div className="min-h-screen bg-slate-50 px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl">
        <div className="mb-10 text-center">
          <div className="mb-6 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-700 text-white shadow-lg">
        ...
        <div className="mb-12">
          <div className="relative flex items-center justify-between">
            ...
            <div className="absolute left-0 top-1/2 -z-10 h-0.5 -translate-y-1/2 bg-indigo-700 transition-all duration-500" style={{ width: `${((step - 1) / 3) * 100}%` }} />
            {steps.map((item) => (
              <div key={item.id} className="flex flex-col items-center">
                <div className={`flex h-10 w-10 items-center justify-center rounded-full border-4 font-semibold transition-colors ${step >= item.id ? 'border-indigo-100 bg-indigo-700 text-white' : 'border-slate-100 bg-white text-slate-400'}`}>
                  <item.icon className="h-4 w-4" />
                </div>
                <span className={`mt-2 text-xs font-medium ${step >= item.id ? 'text-indigo-900' : 'text-slate-400'}`}>{item.title}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <CardContent className="p-8 md:p-12">
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
              {step === 1 && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <input value={form.organizationName} onChange={(event) => updateField('organizationName', event.target.value)} className="w-full rounded-lg border border-slate-300 p-3 text-sm outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="请输入营业执照上的完整名称" />
                  </div>
                  <div>
                    <label className="mb-1 block text-sm font-medium text-slate-700">近期合作重点</label>
                    <textarea value={form.projectFocus} onChange={(event) => updateField('projectFocus', event.target.value)} className="h-28 w-full rounded-lg border border-slate-300 p-3 text-sm outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="例如：希望找到 UI 设计、前端开发或 AI 工作流人才..." />
                  </div>
              ...
              {step === 2 && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <input value={form.contactName} onChange={(event) => updateField('contactName', event.target.value)} className="w-full rounded-lg border border-slate-300 p-3 text-sm outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="您的真实姓名" />
                  </div>
                  <div>
                    <label className="mb-1 block text-sm font-medium text-slate-700">职务</label>
                    <input value={form.contactRole} onChange={(event) => updateField('contactRole', event.target.value)} className="w-full rounded-lg border border-slate-300 p-3 text-sm outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="例如：项目负责人、创始人" />
                  </div>
                  <div>
                    <label className="mb-1 block text-sm font-medium text-slate-700">联系电话</label>
                    <input value={form.contactMobile} onChange={(event) => updateField('contactMobile', event.target.value)} className="w-full rounded-lg border border-slate-300 p-3 text-sm outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="用于审核联系" />
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {['远程协作', '快速交付', '长期合作', '需要发票'].map((item) => (
                      <button key={item} type="button" onClick={() => togglePreference(item)} className={`rounded-full border px-4 py-2 text-sm ${form.collaborationPreferences.includes(item) ? 'border-indigo-300 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600'}`}>
                        {item}
                      </button>
                    ))}
                  </div>
              ...
              {step === 4 && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 rounded-xl bg-slate-50 p-6 text-sm">
                    ...
                    <div className="flex justify-between pb-1"><span className="text-slate-500">上传材料</span><span className="font-medium text-indigo-700">{files.length ? `${files.length} 份文件` : form.deferMaterials ? '后补材料' : '未上传'}</span></div>
                  </div>
                </div>
              )}
              ...
              <div className="mt-10 flex justify-between border-t border-slate-100 pt-6">
                ...
                <Button onClick={handleNext} disabled={isSubmitting} className="bg-indigo-700 px-8 hover:bg-indigo-800">
